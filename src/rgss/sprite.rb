...
class Sprite < RGSS::Sprite
  ...
  alias sprite_initialize initialize
  alias sprite_visible_set visible=

  def initialize(*args)
    sprite_initialize(*args)
...
#    base_y = @y-@oy
#    if @viewport
#      destination.entity.setClipRect(@viewport.rect.x, @viewport.rect.y, @viewport.rect.width, @viewport.rect.height)
#      base_x += @viewport.rect.x - @viewport.ox
#      base_y += @viewport.rect.y - @viewport.oy
#    end
#
#      destination.stretch_blt(
#        Rect.new(base_x, base_y, @src_rect.width*@zoom_x, @src_rect.height*@zoom_y),
#        @bitmap,
#        Rect.new(@src_rect.x, @src_rect.y, @src_rect.width, @src_rect.height),
#        opacity,
#        @viewport ? @viewport.z*100 + @z : @z
#    )
#
#    destination.entity.setClipRect(0, 0, 0, 0) if @viewport
#  end
#end