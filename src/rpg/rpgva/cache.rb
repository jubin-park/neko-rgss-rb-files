module RPG
  module Cache
    @cache = {}
    def self.load_bitmap(folder_name, filename, hue = 0, color_key=false)
      path = folder_name + filename
      if not @cache.include?(path) or @cache[path].disposed?
        if filename != ""
          @cache[path] = Bitmap.new(path, color_key)
        else
    ...
    def self.animation(filename, hue)
      self.load_bitmap("Graphics/Animations/", filename, hue, true)
    end
